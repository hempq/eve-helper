    ...
    public function test_ranks_offers_by_isk_per_lp(): void
    {
        ...
        Http::fake([
            'market.fuzzwork.co.uk/*' => Http::response([
                '500' => ['buy' => ['percentile' => 0], 'sell' => ['percentile' => 5_000_000]],
        ...
        // Best first: item A at 4000/LP, then C (same item, 4000/LP but unaffordable), then B at 1500/LP.
        $this->assertSame('Navy Item A', $offers->first()->item);
        $this->assertEqualsWithDelta(4000.0, $offers->first()->iskPerLp, 0.1);
        $this->assertTrue($offers->first()->affordable);

        $b = $offers->firstWhere('item', 'Navy Item B');
        $this->assertEqualsWithDelta(1500.0, $b->iskPerLp, 0.1);
    }
